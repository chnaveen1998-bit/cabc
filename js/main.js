document.addEventListener("DOMContentLoaded", function () {
  ...
  (function () {
    ...
    pdfjsLib.GlobalWorkerOptions.workerSrc =
      "https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.10.130/pdf.worker.min.js";
    ...
    function renderPage(pageNum) {
      pdfDoc
        ...
        .then((page) => {
          ...
          const scale =
            (canvas.parentElement.clientWidth / viewport.width) * 1.0;
    ...
    const fallbackToIframe = (page = 1) => {
      ...
      viewerContainer.innerHTML =
        '<iframe id="pdfFallbackIframe" src="' +
        PDF_URL +
        "#page=" +
        page +
        '" frameborder="0" style="width:100%;height:640px;border:0;display:block" aria-label="PDF fallback viewer"></iframe>';
    };
  ...
  (function () {
    const events = [
      {
        ...
        date: "2025-10-08",
        photos: [
          "images/events/event1/IMG-20251221-WA0029.jpg",
          "images/events/event1/IMG-20251223-WA0009.jpg",
          "images/events/event1/IMG-20251217-WA0094.jpg",
        ],
      },
      {
        id: 2,
        ...
        date: "2025-09-21",
        photos: [
          "images/events/event2/IMG-20251218-WA0008.jpg",
          "images/events/event2/IMG-20251218-WA0009.jpg",
          "images/events/event2/IMG-20251218-WA0021.jpg",
        ],
      },
      {
        id: 3,
        ...
        date: "2025-10-15",
        photos: [
          "images/events/event3/IMG-20251225-WA0042.jpg",
          "images/events/event3/IMG-20251225-WA0043.jpg",
          "images/events/event3/IMG-20251225-WA0044.jpg",
          "images/events/event3/IMG-20251225-WA0045.jpg",
        ],
      },
    ];
    ...
    if (eventsGrid) {
      ...
      <div class="event-meta">${ev.category} · ${ev.date}</div>
    `;

        /* ---------- EXPLORE BUTTON ---------- */
        body.querySelector(".explore-btn").addEventListener("click", (e) => {
          e.stopPropagation();
          openEventGallery(ev);
        });
        ...
        if (ev.photos.length > 1) {
          ...
          let interval = setInterval(() => {
            idx = (idx + 1) % ev.photos.length;
            img.src = ev.photos[idx];
          }, 2500);
    ...
    async function openEventGallery(ev) {
      ...
      const title = document.getElementById("eventGalleryTitle");
    ...
    document.getElementById("closeEventGallery").onclick = closeEventGallery;
    document.getElementById("eventGalleryBackdrop").onclick = closeEventGallery;
    ...
    function renderCalendar(year, month) {
      ...
      for (let d = 1; d <= last.getDate(); d++) {
        ...
        if (has) {
          ...
          evBadge.style.position = "absolute";
          evBadge.style.left = "8px";
          evBadge.style.bottom = "8px";
          evBadge.style.fontSize = "0.75rem";
          evBadge.style.color = "#ef4444";
    ...
    function savePrayers(arr) {
      localStorage.setItem("prayerWall", JSON.stringify(arr));
    }
    ...
    if (prayerForm) {
      prayerForm.addEventListener("submit", async function (e) {
        ...
        try {
          const res = await fetch("/submit/prayer", {
            ...
            body: JSON.stringify({ name, anon, text, ts: Date.now() }),
          });
      ...
      if (loadPrayers().length === 0) {
        const examples = [
          {
            name: "Alice",
            text: "Please pray for my mother who is unwell.",
            anon: false,
            ts: Date.now() - 1000 * 60 * 60 * 24 * 2,
          },
          {
            name: "Bob",
            text: "Pray for our youth ministry outreach.",
            anon: false,
            ts: Date.now() - 1000 * 60 * 60 * 24 * 4,
          },
          {
            name: "Carol",
            text: "Healing and peace for the family.",
            anon: true,
            ts: Date.now() - 1000 * 60 * 60 * 24 * 6,
          },
          {
            name: "Daniel",
            text: "Guidance for our leadership team.",
            anon: false,
            ts: Date.now() - 1000 * 60 * 60 * 24 * 8,
          },
          {
            name: "Eve",
            text: "Thankful for recent blessings.",
            anon: false,
      ...
      function startPrayerScroll() {
        ...
        function tick() {
          if (prayerWall.scrollHeight - prayerWall.clientHeight <= 1) {
            return;
          }
          prayerWall.scrollTop =
            (prayerWall.scrollTop + step) %
            (prayerWall.scrollHeight - prayerWall.clientHeight + 1);
        }
    ...
    if (membershipForm) {
      ...
      function refreshChildIndices() {
        ...
        trs.forEach((tr, i) => {
          tr.querySelector(".child-idx").textContent = String(i + 1);
        });
      }
      function createChildRow(data = {}) {
        ...
        tr.innerHTML = `
            ...
            <td><input class="child-name" type="text" value="${
              data.name || ""
            }"></td>
            <td><input class="child-phone" type="tel" value="${
              data.phone || ""
            }"></td>
            <td><input class="child-age" type="number" value="${
              data.age || ""
            }"></td>
            <td><input class="child-eduocc" type="text" value="${
              data.eduocc || ""
            }"></td>
            <td><input class="child-address" type="text" value="${
              data.address || ""
            }"></td>
            <td><button type="button" class="btn remove-child">✕</button></td>
          `;
        tr.querySelector(".remove-child").addEventListener("click", () => {
          tr.remove();
          refreshChildIndices();
        });
      ...
      if (addChildBtn) {
        addChildBtn.addEventListener("click", () => createChildRow());
      }
      membershipForm.addEventListener("submit", function (e) {
        ...
        if (childrenTbody) {
          childrenTbody.querySelectorAll("tr").forEach((tr) => {
            const c = {
              name:
                (tr.querySelector(".child-name") &&
                  tr.querySelector(".child-name").value) ||
                "",
              phone:
                (tr.querySelector(".child-phone") &&
                  tr.querySelector(".child-phone").value) ||
                "",
              age:
                (tr.querySelector(".child-age") &&
                  tr.querySelector(".child-age").value) ||
                "",
              eduocc:
                (tr.querySelector(".child-eduocc") &&
                  tr.querySelector(".child-eduocc").value) ||
                "",
              address:
                (tr.querySelector(".child-address") &&
                  tr.querySelector(".child-address").value) ||
                "",
            };
        ...
        const appObj = {
          ...
          birthPlace: document.getElementById("memberBirthPlace")
            ? document.getElementById("memberBirthPlace").value
            : "",
          phone: phone,
          email: email,
          address: document.getElementById("memberAddress").value,
          bloodGroup: document.getElementById("memberBloodGroup")
            ? document.getElementById("memberBloodGroup").value
            : "",
          christianStatus: document.getElementById("memberChristianStatus")
            ? document.getElementById("memberChristianStatus").value
            : "",
          baptismPastor: document.getElementById("memberBaptismPastor")
            ? document.getElementById("memberBaptismPastor").value
            : "",
          baptismYear: document.getElementById("memberBaptismYear")
            ? document.getElementById("memberBaptismYear").value
            : "",
          education: document.getElementById("memberEducation")
          ...
          occupation: document.getElementById("memberOccupation")
            ? document.getElementById("memberOccupation").value
            : "",
          aadhar: document.getElementById("memberAadhar")
            ? document.getElementById("memberAadhar").value
            : "",
          fatherName: document.getElementById("memberFatherName")
            ? document.getElementById("memberFatherName").value
            : "",
          fatherOccupation: document.getElementById("memberFatherOcc")
            ? document.getElementById("memberFatherOcc").value
            : "",
          motherName: document.getElementById("memberMotherName")
            ? document.getElementById("memberMotherName").value
            : "",
          motherOccupation: document.getElementById("memberMotherOcc")
            ? document.getElementById("memberMotherOcc").value
            : "",
          spouseName: document.getElementById("memberSpouseName")
            ? document.getElementById("memberSpouseName").value
            : "",
          spouseOccupation: document.getElementById("memberSpouseOcc")
            ? document.getElementById("memberSpouseOcc").value
            : "",
          ...
          declaration: document.getElementById("memberDeclaration")
            ? document.getElementById("memberDeclaration").value
            : "",
          declarationDate: document.getElementById("memberDeclarationDate")
            ? document.getElementById("memberDeclarationDate").value
            : "",
          declarationPlace: document.getElementById("memberDeclarationPlace")
            ? document.getElementById("memberDeclarationPlace").value
            : "",
          signature: document.getElementById("memberSignature")
            ? document.getElementById("memberSignature").value
            : "",
        };
    ...
    const DRIVE_FOLDER_URL =
      "https://drive.google.com/drive/folders/1YZ6M9QNb0MIaNq1-gRfuWJ-_4VcsXLPh?usp=share_link";
    ...
    if (exportPrayersBtn) {
      exportPrayersBtn.addEventListener("click", async () => {
        ...
        if (!prayers || prayers.length === 0)
          return alert("No prayers to export");
  ...
  //   // 👉 MANUAL VIDEO LIST (you can edit anytime)
  //   const sermons = [
  //     {
  //       id: "l0XAcTSTius",
  ...
  //     {
  //       id: "WAATW7X64Cc",
  //       title: "Special Worship Message",
  //     },
  //     {
  //       id: "sGO4WUeYoms",
  //       title: "143rd Women Conference",
  //     },
  //   ];
  ...
  //   // Load first (latest) video
  //   setMainVideo(sermons[0].id);
  ...
  (function () {
    ...
    const sermons = [
      { id: "l0XAcTSTius", title: "Sunday Worship Service" },
      { id: "P9gUl-bySiw", title: "Church Sermon" },
      { id: "WAATW7X64Cc", title: "Special Worship Message" },
      { id: "sGO4WUeYoms", title: "143rd Women Conference" },
    ];
  ...
  (function () {
    ...
    function ensureLogoutLink() {
      ...
      logoutLink.innerHTML =
        '<span class="lang-te">లాగ్ అవుట్</span><span class="lang-en">Logout</span>';
    ...
    async function setupGoogle() {
      try {
        ...
        console.warn("Failed to setup Google Sign-In", e);
      }
    }
    ...
    if (form) {
      form.addEventListener("submit", function (ev) {
        ...
        fetch("/api/memberships", {
          ...
          .catch(() => showAck("<strong>Error:</strong> Network error", true))
  ...
  ========================= */
    function closeSongModal() {
      ...
      // var input = document.getElementById("songNumber");
      // if (input) input.value = "";
    }

    /* =========================
     SONG FORMATTER (GLOBAL)
  ========================= */
    function formatSongLyrics(text) {
      if (!text) return "";
  ...
  ========================= */
    ...
    if (goBtn) goBtn.addEventListener("click", loadSongFromXML);

    async function loadSongFromXML(ev) {
      ev.preventDefault();

      var val = document.getElementById("songNumber")?.value.trim();
      ...
      try {
        ...
        const title =
          foundSong.getElementsByTagName("name")[0]?.textContent ||
          "Song " + val;
        const lyrics =
          foundSong.getElementsByTagName("slide")[0]?.textContent || "";

        document.getElementById("songModalTitle").textContent = title;
        document.getElementById("songContent").innerHTML =
          formatSongLyrics(lyrics);
        ...
        console.error(err);
        alert("Unable to load song data.");
      }
    }

    var closeBtn = document.getElementById("songCloseBtn");
    if (closeBtn) closeBtn.addEventListener("click", closeSongModal);

    var backdrop = document.getElementById("songBackdrop");
    if (backdrop) backdrop.addEventListener("click", closeSongModal);
  })();
  ...
  (function () {
    ...
    function mkCard(m) {
      ...
      photo.className = "m-photo";
      const img = document.createElement("img");
      img.src =
        m.Photo || m.photo || "uploads/Crucifixion-of-Jesus_20251016163744.jpg";
      ...
      h.innerHTML = `<span class=\"lang-te\">${
        m["Name (TE)"] || m["NameTE"] || m.Name || ""
      }</span><span class=\"lang-en\">${
        m["Name (EN)"] || m["NameEN"] || m.Name || ""
      }</span>`;
      const meta = document.createElement("div");
      meta.className = "m-meta";
      const baptized = m["Baptism Date"] || m.Baptism || m.Baptized || "";
      const memberSince =
        m["Member Since"] || m["Church Member Since"] || m.Since || "";
      meta.textContent = `Baptized: ${baptized} · Member since: ${memberSince}`;
      const contact = document.createElement("div");
      contact.innerHTML = `<div><strong>Email:</strong> ${
        m.Email || m.email || ""
      }</div><div><strong>Phone:</strong> ${m.Phone || m.phone || ""}</div>`;
      info.appendChild(h);
    ...
    function renderMembers(list) {
      ...
      if (!list.length) {
        setStatus("No members match your search");
        return;
      }
